...
function Auth() {
    ...
    const [token, setToken] = useCookies([API.TOKEN_NAME]);

    useEffect(() => {
        if (token[API.TOKEN_NAME]) window.location.href = 'movies/';
    }, [token])

    const loginClicked = () => {
        API.loginUser({ username, password })
            .then((response) => setToken(API.TOKEN_NAME, response.token))
        ...
        <div className="App">
            ...
            <div className="login-container">
                ...
                <input id="username" type="text" placeholder='Username'
                    value={username} onChange={evt => setUsername(evt.target.value)} /><br />
